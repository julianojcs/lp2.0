	...
	* --------------------------------------------------*/
    ...
    jQuery(window).load(function () {
        ...
        jQuery('#request_form .btn-right').click(function () {
            ...
            var rq_specialty = $('#rq_specialty').val();
            var rq_phone = $('#rq_phone').val();
            var rq_city = $('#rq_city').val();
            var rq_state = $('#rq_state').val();
            var rq_crm = $('#rq_crm').val();
            var rq_ufcrm = $('#rq_ufcrm').val();

            if (rq_step == 1) {
                if (rq_name.length == 0) { $('#rq_name').addClass("error_input"); } else { $('#rq_name').removeClass("error_input"); }
                if (rq_email.length == 0) { $('#rq_email').addClass("error_input"); } else { $('#rq_email').removeClass("error_input"); }
                if (rq_specialty.length == 0) { $('#rq_specialty').addClass("error_input"); } else { $('#rq_specialty').removeClass("error_input"); }
                if (rq_phone.length == 0) { $('#rq_phone').addClass("error_input"); } else { $('#rq_phone').removeClass("error_input"); }
                if (rq_city.length == 0) { $('#rq_city').addClass("error_input"); } else { $('#rq_city').removeClass("error_input"); }
                if (rq_state.length == 0) { $('#rq_state').addClass("error_input"); } else { $('#rq_state').removeClass("error_input"); }
                if (rq_crm.length == 0) { $('#rq_crm').addClass("error_input"); } else { $('#rq_crm').removeClass("error_input"); }
                if (rq_ufcrm.length == 0) { $('#rq_ufcrm').addClass("error_input"); } else { $('#rq_ufcrm').removeClass("error_input"); }
            }

            if (rq_name.length != 0 && rq_email.length != 0 && rq_specialty.length != 0 && rq_phone.length != 0 && rq_city.length != 0
                && rq_state.length != 0 && rq_crm.length != 0 && rq_ufcrm.length != 0) {
